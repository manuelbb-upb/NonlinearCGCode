...
struct SP1 <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{SP1})
		return (
			"sefriouiNashGeneticAlgorithms2000",
...
# ╠═╡ skip_as_script = true
#=╠═╡
_sp1_has_refs && Markdown.parse(printrefs(SP1))
  ╠═╡ =#

# ╔═╡ db885ad3-097c-4603-a798-9b2aa29b9db8
begin
	@addmethod num_variables(::SP1) = 2
	@addmethod num_objectives(::SP1) = 2
	@addmethod function lower_variable_bounds(tp::SP1)
		return fill(-100.0, num_variables(tp))
	end
	@addmethod function upper_variable_bounds(tp::SP1)
		return fill(100.0, num_variables(tp))
	end
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::SP1, x)
	n = num_variables(tp)

	y[1] = (x[1] - 1)^2 + (x[1] - x[2])^2
	y[2] = (x[2] - 3)^2 + (x[1] - x[2])^2

    return nothing
end

# ╔═╡ 83e40115-b9e5-4569-9d15-07ed49088f4a
@addmethod function in_place_jac!(Dy, tp::SP1, x)
	Dy .= 0
