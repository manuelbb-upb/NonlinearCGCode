...
struct IKK1 <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{IKK1})
		return (
			"ikedaFailureParetobasedMOEAs2001",
...
# ╠═╡ skip_as_script = true
#=╠═╡
_ikk1_has_refs && Markdown.parse(printrefs(IKK1))
  ╠═╡ =#

# ╔═╡ f35fe19c-ba76-446d-839a-665ca5dbad5f
@addmethod num_variables(::IKK1) = 2

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::IKK1) = 3

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(tp::IKK1)
	return fill(-50.0, num_variables(tp))
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(tp::IKK1)
	return fill(50.0, num_variables(tp))
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::IKK1, x)
	y[1] = x[1]^2
	y[2] = (x[1] - 20)^2
	y[3] = x[2]^2
    return nothing
...
@addmethod function in_place_jac!(Dy, tp::IKK1, x)
	Dy .= 0
	Dy[1, 1] = 2*x[1]
	Dy[2, 1] = 2 * (x[1] - 20)
	Dy[3, 2] = 2*x[2]
