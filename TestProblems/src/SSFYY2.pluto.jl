...
struct SSFYY2 <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{SSFYY2})
		return (
			"shimParetobasedContinuousEvolutionary2002",
			"hubandReviewMultiobjectiveTest2006",
			"chenConjugateGradientMethods2024"
		)
	end
	_ssfyy2_has_refs = true
end

# ╔═╡ 9defcaaf-1b35-4e5f-b775-2ce944756fba
# ╠═╡ skip_as_script = true
#=╠═╡
_ssfyy2_has_refs && Markdown.parse(printrefs(SSFYY2))
  ╠═╡ =#

# ╔═╡ db885ad3-097c-4603-a798-9b2aa29b9db8
begin
	@addmethod num_variables(::SSFYY2) = 1
	@addmethod num_objectives(::SSFYY2) = 2
	@addmethod function lower_variable_bounds(tp::SSFYY2)
		return [-100.0,]
	end
	@addmethod function upper_variable_bounds(tp::SSFYY2)
		return [100.0,]
	end
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::SSFYY2, x)
	_x = first(x)
	
	y[1] = 10 + _x^2 - 10 * cos( π / 2 * _x )
	y[2] = (_x - 4)^2
...
@addmethod function in_place_jac!(Dy, tp::SSFYY2, x)
	Dy .= 0

	_x = first(x)

	Dy[1, 1] = 2 * _x + 10 * π / 2 * sin( π / 2 * _x )
	Dy[2, 1] = 2 * (_x - 4)
