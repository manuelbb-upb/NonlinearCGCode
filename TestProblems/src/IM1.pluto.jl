...
struct IM1 <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{IM1})
		return (
			"ishibuchiMultiobjectiveGeneticLocal1998",
...
# ╠═╡ skip_as_script = true
#=╠═╡
_im1_has_refs && Markdown.parse(printrefs(IM1))
  ╠═╡ =#

# ╔═╡ f35fe19c-ba76-446d-839a-665ca5dbad5f
@addmethod num_variables(::IM1) = 2

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::IM1) = 2

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(tp::IM1)
	return [1.0, 1.0]
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(tp::IM1)
	return [4.0, 2.0]
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::IM1, x)
	y[1] = 2 * sqrt( x[1] )
	y[2] = x[1] * (1 - x[2]) + 5
    return nothing
end
...
@addmethod function in_place_jac!(Dy, tp::IM1, x)
	Dy .= 0
	Dy[1, 1] = 1 / (x[1]^0.5)
	Dy[2, 1] = (1 - x[2])
	Dy[2, 2] = (-x[1])
