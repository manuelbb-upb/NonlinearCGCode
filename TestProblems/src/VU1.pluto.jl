...
struct VU1 <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{VU1})
		return (
			"rendon1997non",
...
# ╠═╡ skip_as_script = true
#=╠═╡
_vu1_has_refs && Markdown.parse(printrefs(VU1))
  ╠═╡ =#

# ╔═╡ db885ad3-097c-4603-a798-9b2aa29b9db8
begin
	@addmethod num_variables(::VU1) = 2
	@addmethod num_objectives(::VU1) = 2
	@addmethod function lower_variable_bounds(tp::VU1)
		return fill(-3.0, num_variables(tp))
	end
	@addmethod function upper_variable_bounds(tp::VU1)
		return fill(3.0, num_variables(tp))
	end
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::VU1, x)
	n = num_variables(tp)
...
@addmethod function in_place_jac!(Dy, tp::VU1, x)
	Dy .= 0

	Dy[1, :] .= (-2 / (sum(x.^2) + 1)^2) .* x
