...
md"# RB2D -- 2D Rosenbrock Problem"

# ╔═╡ 3427a00c-f610-4eee-bb71-15d53d3e5b48
md"## Single-Objective"

# ╔═╡ 5babed3c-00f2-4b0a-9d89-dcede5363a73
md"""
...
# ╔═╡ b0e94e73-3a7a-4414-a687-815fe1619cfb
md"""
...
# ╔═╡ 14ecc35f-9fee-4173-a6b4-0efbc7730104
function f_rosenbrock(x1, x2, a, b)
	return b * (x2 - x1^2)^2 + (a - x1)^2
end

# ╔═╡ 47bc5467-f890-467a-9288-1eeab7b58128
function df_rosenbrock(x1, x2, a, b) 
	return [
...
# ╔═╡ 325bfac4-58de-40aa-af73-6968814b6c2f
md"## Bi-Objective Problem"

# ╔═╡ aa80bf99-2f84-4af0-b085-1703047f1885
md"""
...
Base.@kwdef struct RB2D <: AbstractTestProblem
	a1 :: Float64 = 1
	a2 :: Float64 = 2 * a1
	b1 :: Float64 = 100
	b2 :: Float64 = b1
...
@addmethod num_variables(::RB2D) = 2

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::RB2D) = 2

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(tp::RB2D)
	return [-5.0, -5.0]
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(tp::RB2D)
	return [5.0, 5.0]
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::RB2D, x)
	a1 = tp.a1
	a2 = tp.a2
	b1 = tp.b1
	b2 = tp.b2
	y[1] = f_rosenbrock(x[1], x[2], a1, b1)
	y[2] = f_rosenbrock(x[1], x[2], a2, b2)
    return nothing
end

# ╔═╡ 5d4bacb1-0a57-462f-9119-a9d5b4263103
@addmethod function in_place_jac!(Dy, tp::RB2D, x)
	a1 = tp.a1
	a2 = tp.a2
	b1 = tp.b1
	b2 = tp.b2

	Dy[1, :] .= df_rosenbrock(x[1], x[2], a1, b1)
	Dy[2, :] .= df_rosenbrock(x[1], x[2], a2, b2)
