...
struct PNR <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{PNR})
		return (
			"preussParetoSetEMOA2006",
...
# ╠═╡ skip_as_script = true
#=╠═╡
_pnr_has_refs && Markdown.parse(printrefs(PNR))
  ╠═╡ =#

# ╔═╡ db885ad3-097c-4603-a798-9b2aa29b9db8
begin
	@addmethod num_variables(::PNR) = 2
	@addmethod num_objectives(::PNR) = 2
	@addmethod function lower_variable_bounds(tp::PNR)
		return fill(-2.0, num_variables(tp))
	end
	@addmethod function upper_variable_bounds(tp::PNR)
		return fill(2.0, num_variables(tp))
	end
end

# ╔═╡ 70cd7853-61c4-4640-a1c4-45815118f456
function _pnr_params()
	c = 10.0
	d = 0.25
	k = 0.0
	l = 1.0
...
@addmethod function in_place_objectives!(y, tp::PNR, x)
	c, d, k, l = _pnr_params()
	
	y[1] = sum( x.^4 ) - x[1]^2 + x[2]^2 - c * prod(x) + d * x[1] + 20
	y[2] = (x[1] - k)^2 + (x[2] - l)^2
...
@addmethod function in_place_jac!(Dy, tp::PNR, x)
	Dy .= 0
