...
abstract type AbstractMGHTestProblem <: AbstractTestProblem end

# ╔═╡ 3e81e7a1-b44d-4f7b-95d7-ea54bb71f3fa
md"Adapted from single-objective optimization test problems."
...
begin
	@addmethod function reference_keys(::Type{<:AbstractMGHTestProblem})
		return (
			"moreTestingUnconstrainedOptimization1981",
...
struct MGH16 <: AbstractMGHTestProblem end

# ╔═╡ 9defcaaf-1b35-4e5f-b775-2ce944756fba
# ╠═╡ skip_as_script = true
#=╠═╡
_mgh_has_refs && Markdown.parse(printrefs(MGH16))
  ╠═╡ =#

# ╔═╡ abca6c2f-30ff-4bdf-aeba-8a928726359d
begin
	@addmethod num_variables(::MGH16) = 4
	@addmethod num_objectives(::MGH16) = 5
	@addmethod function lower_variable_bounds(tp::MGH16)
		return [-25.0, -5.0, -5.0, -1.0]
	end
	@addmethod function upper_variable_bounds(tp::MGH16)
		return -lower_variable_bounds(tp)
	end
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::MGH16, x)
	for i = eachindex(y)
		ti = i/5
		y[i] = (x[1] + ti * x[2] - exp(ti))^2 + (x[3] + x[4] * sin(ti) - cos(ti))^2
	end
    return nothing
end

# ╔═╡ 83e40115-b9e5-4569-9d15-07ed49088f4a
@addmethod function in_place_jac!(Dy, tp::MGH16, x)
	Dy .= 0
	for i = axes(Dy, 1)
		ti = i/5
		Dy[i, 1] = Dy[i, 2] = 2 * (x[1] + ti * x[2] - exp(ti))
		Dy[i, 2] *= ti
		Dy[i, 3] = Dy[i, 4] = 2 * (x[3] + x[4] * sin(ti) - cos(ti))
		Dy[i, 4] *= sin(ti)
	end
    return nothing
end
...
struct MGH26 <: AbstractMGHTestProblem end

# ╔═╡ 1f164705-2181-4369-ba70-ae488fc8b7cb
begin
	@addmethod num_variables(::MGH26) = 4
	@addmethod num_objectives(::MGH26) = 4
	@addmethod function lower_variable_bounds(tp::MGH26)
		return fill(-1.0, num_variables(tp))
	end
	@addmethod function upper_variable_bounds(tp::MGH26)
		return -lower_variable_bounds(tp)
	end
end

# ╔═╡ 1039c66f-7fb1-451b-bf9e-b8b512370af2
@addmethod function in_place_objectives!(y, tp::MGH26, x)
	n = num_variables(tp)
	c = sum( cos(x[j]) for j=1:n )
	for i = eachindex(y)
		y[i] = ( n - c + i * (1 - cos(x[i])) + sin(x[i]) )^2
	end
    return nothing
end

# ╔═╡ 7ce76189-4024-4857-b7e7-ac6cdab3a27d
@addmethod function in_place_jac!(Dy, tp::MGH26, x)
	Dy .= 0
	n = num_variables(tp)
	c = sum( cos(x[j]) for j=1:n )
	for i = axes(Dy, 1)
		Dy[i, :] .= 2 * ( n - c + i * (1 - cos(x[i])) + sin(x[i]) )
		for j = axes(Dy, 2)
			i == j && continue
			Dy[i, j] *= sin(x[j])
		end
		Dy[i, i] *= ( (1 + i) * sin(x[i]) + cos(x[i]) )
	end
    return nothing
end
