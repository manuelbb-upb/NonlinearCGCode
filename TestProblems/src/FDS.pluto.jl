...
Base.@kwdef struct FDS <: AbstractTestProblem
	num_vars :: Int = 5
end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{FDS})
		return (
...
# ╠═╡ skip_as_script = true
#=╠═╡
_fds_has_refs && Markdown.parse(printrefs(FDS))
  ╠═╡ =#

# ╔═╡ 99531065-6a13-4269-9897-2062a5263ddb
begin
# There is some confusion concerning this enumeration.
# I use the variable values found in the origininal reference.
function FDSa()
	return FDS(5)
end
function FDSb()
...
@addmethod num_variables(tp::FDS) = tp.num_vars

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::FDS) = 3

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(tp::FDS)
	return fill(-2.0, num_variables(tp))
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(tp::FDS)
	return fill(2.0, num_variables(tp))
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::FDS, x)
	n = num_variables(tp)
	y[1] = (1 / n^2) * sum( k * (x[k] - k)^4 for k=1:n )
	y[2] = exp( 1/n * sum( x[k] for k=1:n ) ) + sum( x.^2 )
	y[3] = (1 / (n *(n + 1))) * sum( k * (n - k + 1) * exp(-x[k]) for k=1:n)
    return nothing
end

# ╔═╡ 83e40115-b9e5-4569-9d15-07ed49088f4a
@addmethod function in_place_jac!(Dy, tp::FDS, x)
	n = num_variables(tp)
	Dy[1, :] .= ((1 / n^2) * 4) .* [ k * (x[k] - k)^3 for k=1:n ]
	Dy[2, :] .= 2 .* x .+
		(1/n) .* exp( 1/n * sum( x[k] for k=1:n ) ) .* ones(n)
	Dy[3, :] .= (1 / (n *(n + 1))) .* [
		-k * (n - k + 1) * exp(-x[k]) for k=1:n
	]
    return nothing
end
