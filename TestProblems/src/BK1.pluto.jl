...
# ╔═╡ eab9227d-4ec2-4c8e-88be-e67815c36277
md"References:"

# ╔═╡ 225b9218-3c17-4489-a030-12c95d1ab001
md"Simple bi-objective test problem with finite parameter intervals."

# ╔═╡ 6568a27d-b790-48fd-82c6-27ebb403c494
md"""
```math
\begin{aligned}
...
&\text{s.t.}\\
&\mathbf x \in [-5, 10]^2, \\
&f1(\mathbf x) = x_1^2 + x_2^2 \\
&f2(\mathbf x) = (x_1 - 5)^2 + (x_2 - 5)^2
\end{aligned}
```
"""

# ╔═╡ fe514db9-330b-40e6-8258-f4e908bece52
struct BK1 <: AbstractTestProblem end

# ╔═╡ f4634d3d-a126-4167-9f86-ed789d4edb59
begin
	@addmethod function reference_keys(::Type{BK1})
		return (
			"binhEvolutionStrategyMultiobjective1996",
...
# ╔═╡ 1b8820de-86fc-41d7-affb-0cda4c16ed9e
# ╠═╡ skip_as_script = true
#=╠═╡
_bk1_has_refs && Markdown.parse(printrefs(BK1))
  ╠═╡ =#

# ╔═╡ f35fe19c-ba76-446d-839a-665ca5dbad5f
@addmethod num_variables(::BK1) = 2

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::BK1) = 2

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(::BK1)
	return [-5.0, -5.0]
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(::BK1)
	return [10.0, 10.0]
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::BK1, x)
	y[1] = sum( x.^2 )
	y[2] = sum( (x .- 5).^2 )
    return nothing
end
...
@addmethod function in_place_jac!(Dy, tp::BK1, x)
	Dy[1, :] .= 2 .* x
	Dy[2, :] .= 2 .* (x .- 5)
    return nothing
end
