...
abstract type AbstractSLCDTTestProblem <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{<:AbstractSLCDTTestProblem})
		return (
			"schutzeConvergenceStochasticSearch2008",
			"chenConjugateGradientMethods2024"
		)
	end
	_slcdt_has_refs = true
end

# ╔═╡ fe514db9-330b-40e6-8258-f4e908bece52
struct SLCDT2 <: AbstractSLCDTTestProblem end

# ╔═╡ 9defcaaf-1b35-4e5f-b775-2ce944756fba
# ╠═╡ skip_as_script = true
#=╠═╡
_slcdt_has_refs && Markdown.parse(printrefs(SLCDT2))
  ╠═╡ =#

# ╔═╡ db885ad3-097c-4603-a798-9b2aa29b9db8
begin
	@addmethod num_variables(::SLCDT2) = 10
	@addmethod num_objectives(::SLCDT2) = 2
	@addmethod function lower_variable_bounds(tp::SLCDT2)
		return fill(-1.0, num_variables(tp))
	end
	@addmethod function upper_variable_bounds(tp::SLCDT2)
		return fill(1.0, num_variables(tp))
	end
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::SLCDT2, x)
	n = num_variables(tp)

	y[1] = (x[1] - 1)^4 + sum( (x[2:end] .- 1).^2 )
	y[2] = (x[2] + 1)^4 + sum( (x[[1:1; 3:end]] .+ 1).^2 )
	
    return nothing
end

# ╔═╡ 83e40115-b9e5-4569-9d15-07ed49088f4a
@addmethod function in_place_jac!(Dy, tp::SLCDT2, x)
	Dy .= 0

	Dy[1, 1] = 4 * (x[1] - 1)^3
	Dy[1, 2:end] .= 2 .* (x[2:end] .- 1)

	Dy[2, 2] = 4 * (x[2] + 1)^3
	Dy[2, [1:1; 3:end]] .= 2 .* (x[[1:1; 3:end]] .+ 1)
    return nothing
end
