...
# ╔═╡ 42529030-0f0f-41af-9647-071a775d0340
md"References:"

# ╔═╡ 4c47ae39-279f-48c1-bd20-2f2709911d64
md"## JOS1"

# ╔═╡ cae9634b-06da-4843-8597-e45415eebed7
md"Simple bi-objective problem with quadratic objectives."

# ╔═╡ fe514db9-330b-40e6-8258-f4e908bece52
Base.@kwdef struct JOS1 <: AbstractTestProblem
	num_vars :: Int = 10
end

# ╔═╡ 2df0e294-a033-456e-a4a3-be8c2919d562
begin
	@addmethod function reference_keys(::Type{JOS1})
		return (
			"jinDynamicWeightedAggregation",
...
# ╔═╡ e0f7b61b-a232-4886-9248-b0ad90cd0143
# ╠═╡ skip_as_script = true
#=╠═╡
_jos1_has_refs && Markdown.parse(printrefs(JOS1))
  ╠═╡ =#

# ╔═╡ ba92cefd-7c71-452f-bafc-b3da75718fd4
begin
	function JOS1a()
		return JOS1(50)
	end
...
@addmethod num_variables(tp::JOS1) = tp.num_vars

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::JOS1) = 2

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(tp::JOS1)
	return fill(-100.0, num_variables(tp))
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(tp::JOS1)
	return fill(100.0, num_variables(tp))
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::JOS1, x)
	n = num_variables(tp)
	y[1] = (1/n) * sum( x.^2 )
	y[2] = (1/n) * sum( (x .- 2).^2 )
    return nothing
...
@addmethod function in_place_jac!(Dy, tp::JOS1, x)
	n = num_variables(tp)
	Dy[1, :] .= (2/n) .* x
	Dy[2, :] .= (2/n) .* (x .- 2)
    return nothing
