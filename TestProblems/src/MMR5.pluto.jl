...
# ╔═╡ 6156365d-433f-4cc0-94e5-5a6baa410c35
abstract type AbstractMMRTestProblem <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{<:AbstractMMRTestProblem})
		return (
			"miglierinaBoxconstrainedMultiobjectiveOptimization2008",
...
# ╔═╡ db885ad3-097c-4603-a798-9b2aa29b9db8
 begin
	@addmethod num_variables(tp::MMR5) = tp.num_vars
	@addmethod num_objectives(::MMR5) = 2
	@addmethod function lower_variable_bounds(tp::MMR5)
		return fill(-5.0, num_variables(tp))
	end
	@addmethod function upper_variable_bounds(tp::MMR5)
		return fill(5.0, num_variables(tp))
	end
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::MMR5, x)
	n = num_variables(tp)
	
	y[1] = (1/n * sum( x.^2 - 10 .* cos.(2*π .* x) .+ 10 ))^0.25
	_x = x .- 1.5
	y[2] = (1/n * sum( _x.^2 - 10 .* cos.(2*π .* _x) .+ 10 ))^0.25
	
    return nothing
end

# ╔═╡ 83e40115-b9e5-4569-9d15-07ed49088f4a
@addmethod function in_place_jac!(Dy, tp::MMR5, x)
	Dy .= 0

	n = num_variables(tp)
	_x = x .- 1.5
	Dy[1, :] .= 0.25 / (1/n * sum( x.^2 - 10 .* cos.(2*π .* x) .+ 10 ))^0.75 	
	Dy[1, :] .*= 1/n * (2 .* x + 10 * 2 * π * sin.(2*π .* x))
	Dy[2, :] .= 0.25 / (1/n * sum( _x.^2 - 10 .* cos.(2*π .* _x) .+ 10 ))^0.75 	
	Dy[2, :] .*= 1/n * (2 .* _x + 10 * 2 * π * sin.(2*π .* _x))
    return nothing
end
