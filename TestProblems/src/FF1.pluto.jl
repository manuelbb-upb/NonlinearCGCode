...
struct FF1 <: AbstractTestProblem end

# ╔═╡ 723b9650-eab5-454b-85cf-cf7c7cbc27c4
begin
	@addmethod function reference_keys(::Type{FF1})
		return (
			"fonsecaOverviewEvolutionaryAlgorithms1995",
...
# ╠═╡ skip_as_script = true
#=╠═╡
_ff1_has_refs && Markdown.parse(printrefs(FF1))
  ╠═╡ =#

# ╔═╡ f35fe19c-ba76-446d-839a-665ca5dbad5f
@addmethod num_variables(::FF1) = 2

# ╔═╡ 06fcc024-3a5c-4387-bfb2-f0db270d74c5
@addmethod num_objectives(::FF1) = 2

# ╔═╡ 6baaa190-521f-4e7a-a889-637c56ffedf7
@addmethod function lower_variable_bounds(tp::FF1)
	return fill(-1.0, num_variables(tp))
end

# ╔═╡ 26701ed0-1ceb-4db5-a5e2-98449edc112b
@addmethod function upper_variable_bounds(tp::FF1)
	return fill(1.0, num_variables(tp))
end

# ╔═╡ bae8297d-a089-4777-a67c-5cc15d5c8972
@addmethod function in_place_objectives!(y, tp::FF1, x)
	y[1] = 1 - exp( -(x[1] - 1)^2 - (x[2] + 1)^2)	
	y[2] = 1 - exp( -(x[1] + 1)^2 - (x[2] - 1)^2)
    return nothing
end

# ╔═╡ 83e40115-b9e5-4569-9d15-07ed49088f4a
@addmethod function in_place_jac!(Dy, tp::FF1, x)
	y = zeros(num_objectives(tp))
